...
),  __dbt__cte__pokemon_moves_version_group_details_ab2 as (
...
-- version_group_details at pokemon/moves/version_group_details
where 1 = 1
),  __dbt__cte__pokemon_moves_version_group_details_ab3 as (

-- SQL model to build a hash column based on the values of this record
-- depends_on: __dbt__cte__pokemon_moves_version_group_details_ab2
select
    md5(cast(coalesce(cast(_airbyte_moves_hashid as text), '') || '-' || coalesce(cast(level_learned_at as text), '') || '-' || coalesce(cast(version_group as text), '') || '-' || coalesce(cast(move_learn_method as text), '') as text)) as _airbyte_version_group_details_hashid,
    tmp.*
from __dbt__cte__pokemon_moves_version_group_details_ab2 tmp
-- version_group_details at pokemon/moves/version_group_details
where 1 = 1
)-- Final base SQL model
-- depends_on: __dbt__cte__pokemon_moves_version_group_details_ab3
select
    _airbyte_moves_hashid,
    level_learned_at,
    version_group,
...
from __dbt__cte__pokemon_moves_version_group_details_ab3
-- version_group_details at pokemon/moves/version_group_details from "postgres".public."pokemon_moves"
where 1 = 1